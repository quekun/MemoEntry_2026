...
void AFlexibleWall::AttemptToRaise()
{
    ...
    FVector CheckLocation = Mover->StartLocation + FVector(0, 150.f, 120.0f);

    // 稍微把盒子减薄一点，进一步降低碰到地板的概率
    FVector BoxHalfSize = FVector(40.f, 250.f, 80.f);

    //DrawDebugBox(GetWorld(), CheckLocation, BoxHalfSize, FColor::Green, false, 0.5f, 0, 2.0f);

    TArray<FOverlapResult> OutOverlaps;
    // 不要用 AllObjects 初始化！直接指定只查 Pawn
    FCollisionObjectQueryParams ObjectQueryParams;
    ObjectQueryParams.AddObjectTypesToQuery(ECC_Pawn);
    // 或者更简短的写法：FCollisionObjectQueryParams ObjectQueryParams(ECC_Pawn);
