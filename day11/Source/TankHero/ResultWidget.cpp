...
void UResultWidget::SetupResult(bool bIsWin, float TimeSpent)
{
    ...
    {
        GameInst->CurrentSaveGame->SaveCurrentLevel(Tank->Flawless, Tank->Noitem, Tank->Speedrun, GameInst->CurrentLevelIndex);
        if (GameInst->CurrentSaveGame->MaxLevel < GameInst->MaxUnlockedLevelIndex)
        {
            GameInst->CurrentSaveGame->MaxLevel = GameInst->MaxUnlockedLevelIndex;
        }
    }
